...
typedef std::map<std::string, std::pair<std::function<double(double* args)>, std::size_t>> fmap_type;
...
struct interpreter_impl
{
    ...
    interpreter_impl(fmap_type const& fmap, error_handler_type const& error_handler)
        : fmap(fmap), error_handler(error_handler)
    {
    }

    double operator()(ast::nil) const
    {
        assert(0);
        return 0.0;
    }
...
double interpreter_impl::operator()(double ast) const { return ast; }
...
double interpreter_impl::operator()(double lhs, ast::operation const& ast) const
{
    ...
    switch (ast.operator_)
    {
        ...
        default: BOOST_ASSERT(0); return -1;
    }
}
...
double interpreter_impl::operator()(ast::expression const& ast) const
{
    ...
    for (auto const& oper : ast.rest) r = (*this)(r, oper);
    return r;
}
...
double interpreter_impl::operator()(ast::function_call const& ast) const
{
    ...
    if (iter->second.second != ast.arguments.size())
    {
        ...
        out << "Wrong number of arguments to function " << ast.name << " (" << iter->second.second << " expected)."
            << std::endl;
    ...
    double* p = args;
    for (auto const& arg : ast.arguments) *p++ = (*this)(arg);
...
namespace ast
{
float interpreter::eval(ast::expression const& ast) { return interpreter_impl(fmap, error_handler)(ast); }
}  // namespace ast
}  // namespace fun
