...
class Optimizer {
 public:
  ...
  static auto IsUnusedStatement(ThreeAddressCode const& statement,
                                StatementList const& statementList) -> bool;
  static auto ReplaceVariableIfConstant(ThreeAddressCode& statement,
                                        StatementList& statementList) -> bool;

  // replace constant expression with store of result.
  static auto ReplaceWithConstantStore(ThreeAddressCode& statement) -> bool;
  static auto isConstantArgument(ThreeAddressCode::Argument const& argument)
      -> bool;
  static auto isConstantArgument(
      ThreeAddressCode::OptionalArgument const& argument) -> bool;
  static auto isConstantStoreExpression(ThreeAddressCode const& statement)
      -> bool;
  static auto isConstantBinaryExpression(ThreeAddressCode const& statement)
      -> bool;
