...
public:
    ...
    [[nodiscard]] virtual std::pair<std::size_t, std::size_t> GetSourceLocation() const = 0;
};
...
public:
    ...
    [[nodiscard]] ASTNodeType GetType() const override { return ASTNodeType::Operand; }

    [[nodiscard]] std::vector<ASTNode*> GetChildren() const override { return {}; }

    [[nodiscard]] std::pair<std::size_t, std::size_t> GetSourceLocation() const override
    {
...
public:
    ...
    [[nodiscard]] ASTNodeType GetType() const override { return ASTNodeType::ConstantExpression; }
...
public:
    ASTBinaryExpr(std::unique_ptr<ASTNode> lhs, SyntaxToken op, std::unique_ptr<ASTNode> rhs)
        : lhs_ {std::move(lhs)}
        , operand_ {std::make_unique<ASTOperand>(std::move(op))}
        , rhs_ {std::move(rhs)}
    {
    }
    ...
    [[nodiscard]] ASTNodeType GetType() const override { return ASTNodeType::BinaryExpression; }
...
{
public:
    static auto PrettyPrint(std::ostream& out, ASTNode const& node, std::string indent = "",
                            bool isLast = true) -> void
    {
