...
constexpr auto fibonacci(tcc::Integer x) -> tcc::Integer
{
    if (x < 2) return x;
...
static void BM_CppFibonacci(benchmark::State& state)
{
    for (auto _ : state)
    {
        auto result = fibonacci(ARGUMENT);
        benchmark::DoNotOptimize(result);
    }
}
BENCHMARK(BM_CppFibonacci);
...
static void BM_StackMachineFibonacci(benchmark::State& state)
{
    ...
    auto const assembly = createFibonacciAssembly(ARGUMENT);
...
BENCHMARK(BM_StackMachineFibonacci);
