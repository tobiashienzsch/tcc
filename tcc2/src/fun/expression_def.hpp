...
auto const additive_expr_def
    = multiplicative_expr >> *((char_('+') > multiplicative_expr) | (char_('-') > multiplicative_expr));
...
auto function_call_def = identifier >> -('(' > argument_list > ')');

auto const primary_expr_def = double_ | function_call | '(' > expression > ')';

auto const expression_def = additive_expr;
// EXPRESSION_DEF5_VISIT_END

// EXPRESSION_DEF6_VISIT_BEGIN
BOOST_SPIRIT_DEFINE(expression, additive_expr, multiplicative_expr, unary_expr, primary_expr, argument_list,
                    function_call);
